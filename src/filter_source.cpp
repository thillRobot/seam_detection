...
// A source .pcd file can also be input for cluster selection
...
bool get_source_complete=0;
bool filtered_source_saved=0; 

void get_source_state_callback(const std_msgs::Bool::ConstPtr& msg)
{
  if (!msg->data){
    ROS_INFO("get_source in progress, waiting to begin filtering ...");
  }
  else if (msg->data&&!get_source_complete){
    ROS_INFO("get_source complete, beginning filtering");
    get_source_complete=1;
  }
}
...
void filter_cloud(PointCloud &input, PointCloud &output, double box[], double leaf_size, bool translate_output, bool auto_bounds)
{
  ...
  PointCloud::Ptr cloud (new PointCloud);       //use this as the working copy of the source cloud
...
int main(int argc, char** argv)
{
  ...
  ros::init(argc,argv,"filter_source");
  ros::NodeHandle node;
  ros::Rate loop_rate(2);
  
  // setup subcribers for get_source_state
  ros::Subscriber get_target_state_sub = node.subscribe("/get_source/get_source_state", 1000, get_source_state_callback);

  // publisher for filter_source_state
  ros::Publisher filter_source_state_pub = node.advertise<std_msgs::Bool> ("/filter_source/filter_source_state", 1);
  
  std_msgs::Bool filter_source_state_msg;
  filter_source_state_msg.data=filtered_source_saved;


  std::cout<<"===================================================================="<<std::endl;
  std::cout<<"                     filter_source v1.6                              "<<std::endl;
  std::cout<<"===================================================================="<<std::endl;
  std::cout<<"Using PCL version:"<< PCL_VERSION_PRETTY <<std::endl<<std::endl;

  std::cout<<"===================================================================="<<std::endl;
  std::cout<<"                     filter_source: loading configuration file       "<<std::endl;
  ...
  node.getParam("filter_source/input_file", input_file);
  input_path=packagepath+'/'+input_file;
  node.getParam("filter_source/output_file", output_file);
  output_path=packagepath+'/'+output_file;
  node.getParam("filter_source/target_file", target_file);
  ...
  node.getParam("filter_source/filter_box",  filter_box_vec);
  for(unsigned i=0; i < filter_box_vec.size(); i++)
    filter_box[i]=filter_box_vec[i]; // copy from vector to array 
  node.getParam("filter_source/voxel_leaf_size", voxel_leaf_size);
  ...
  std::cout<<"===================================================================="<<std::endl;
  std::cout<<"                     filter_source: preparing pointcloud data        "<<std::endl;
  std::cout<<"===================================================================="<<std::endl;
  
  // instantiate some cloud pointers
  PointCloud::Ptr source_input (new pcl::PointCloud<pcl::PointXYZ>); 
  PointCloud::Ptr source_filtered (new pcl::PointCloud<pcl::PointXYZ>);
  PointCloud::Ptr source_filtered_target (new pcl::PointCloud<pcl::PointXYZ>); 
  PointCloud::Ptr source_cluster0 (new pcl::PointCloud<pcl::PointXYZ>);
  ...
  if (pcl::io::loadPCDFile<pcl::PointXYZ> (input_path, *source_input) == -1)
  {
      std::cout<<"Couldn't read cloud_input file:"<<input_path;
      return (-1);
  }
  std::cout << "Loaded input cloud file: "<< input_path <<std::endl<<
    source_input->width * source_input->height << " Data points from "<< input_path << std::endl;
  if (pcl::io::loadPCDFile<pcl::PointXYZ> (target_path, *source_filtered_target) == -1)
  {
      std::cout<<"Couldn't read cloud_target file:"<<target_path;
      return (-1);
  }
  std::cout << "Loaded image file: "<< target_path <<std::endl<<
    source_filtered_target->width * source_filtered_target->height << " Data points from "<< target_path << std::endl;  

  std::cout<<"===================================================================="<<std::endl;
  std::cout<<"                    filter_source: processing pointcloud data        "<<std::endl;
  std::cout<<"===================================================================="<<std::endl<<std::endl;

  // Filter the LiDAR cloud with a bounding box and a voxel (downsampling)
  filter_cloud(*source_input,*source_filtered, filter_box, voxel_leaf_size, translate_output, automatic_bounds); 
  
  Eigen::Quaternionf source_quaternion, cluster0_quaternion, cluster1_quaternion, cluster2_quaternion, cluster3_quaternion, cluster4_quaternion, marker_quaternion;
  Eigen::Vector3f source_translation, cluster0_translation, cluster1_translation, cluster2_translation, cluster3_translation, cluster4_translation, marker_translation;
  Eigen::Vector3f source_dimensions, cluster0_dimensions, cluster1_dimensions, cluster2_dimensions, cluster3_dimensions, cluster4_dimensions, marker_dimensions;
  
  //double source_volume, cluster0_volume, cluster1_volume, cluster2_volume, cluster3_volume, cluster4_volume;
  //double source_aspect_ratio, cluster0_aspect_ratio, cluster1_aspect_ratio, cluster2_aspect_ratio, cluster3_aspect_ratio, cluster4_aspect_ratio;

  if(use_clustering){

    // find clusters in filtered cloud (five clusters hardcoded is messy, fix this somehow)
    cluster_cloud(*source_filtered, *source_cluster0, *source_cluster1, *source_cluster2, *source_cluster3, *source_cluster4);

    // find the minimum bounding box for the source cluster
    pcabox_cloud(*source_filtered_target, source_quaternion, source_translation, source_dimensions); 

    // find the minimum bounding box for a five clusters
    pcabox_cloud(*source_cluster0, cluster0_quaternion, cluster0_translation, cluster0_dimensions);
    pcabox_cloud(*source_cluster1, cluster1_quaternion, cluster1_translation, cluster1_dimensions);
    pcabox_cloud(*source_cluster2, cluster2_quaternion, cluster2_translation, cluster2_dimensions);
    pcabox_cloud(*source_cluster3, cluster3_quaternion, cluster3_translation, cluster3_dimensions);
    pcabox_cloud(*source_cluster4, cluster4_quaternion, cluster4_translation, cluster4_dimensions);

    // get score for each cluster
    double score0, score1, score2, score3, score4, score;
    //score=100;
    score0=score_cluster(*source_cluster0, *source_filtered_target);
    score1=score_cluster(*source_cluster1, *source_filtered_target);
    score2=score_cluster(*source_cluster2, *source_filtered_target);
    score3=score_cluster(*source_cluster3, *source_filtered_target);
    score4=score_cluster(*source_cluster4, *source_filtered_target);
  ...
  std::cout<<"===================================================================="<<std::endl;
  std::cout<<"                   filter_source: pointcloud processing complete     "<<std::endl;
  std::cout<<"===================================================================="<<std::endl<<std::endl;

  std::cout<<"===================================================================="<<std::endl;
  std::cout<<"                   filter_source: preparing visualization            "<<std::endl;
  std::cout<<"===================================================================="<<std::endl<<std::endl;

  ros::Publisher pub_input = node.advertise<PointCloud> ("/source_input", 1) ;
  ros::Publisher pub_filtered = node.advertise<PointCloud> ("/source_filtered", 1) ;
  ros::Publisher pub_filtered_target = node.advertise<PointCloud> ("/source_filtered_target", 1) ;
  ros::Publisher pub_cluster0 = node.advertise<PointCloud> ("/source_cluster0", 1) ;
  ros::Publisher pub_cluster1 = node.advertise<PointCloud> ("/source_cluster1", 1) ;
  ...
  source_filtered_target->header.frame_id = "base_link";
  source_cluster0->header.frame_id = "base_link";
  source_cluster1->header.frame_id = "base_link";
  ...
  if (use_clustering){
    ...
    source_marker.pose.orientation.z = source_quaternion.vec()[2];
    source_marker.pose.orientation.w = source_quaternion.w();

    source_marker.scale.x = source_dimensions[0];
    source_marker.scale.y = source_dimensions[1];
    source_marker.scale.z = source_dimensions[2];

    source_marker.pose.position.x = source_translation[0];
  ...
  std::cout<<"===================================================================="<<std::endl;
  std::cout<<"                        filter_source: complete                     "<<std::endl;
  std::cout<<"===================================================================="<<std::endl<<std::endl;
  ...
  while(ros::ok())
  {
      ...
      filter_source_state_msg.data=filtered_source_saved;
      filter_source_state_pub.publish(filter_source_state_msg);

      pub_input.publish(source_input);
      pub_filtered.publish(source_filtered);
      pub_filtered_target.publish(source_filtered_target);
      if(use_clustering){
        pub_cluster0.publish(source_cluster0);
        pub_cluster1.publish(source_cluster1);
